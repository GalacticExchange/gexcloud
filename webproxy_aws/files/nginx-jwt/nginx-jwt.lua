...
local myconfig = require "gex_config"
local secret = myconfig.get_jwt_token_secret()
local proxy_domainname = myconfig.get_domainname()
...
function M.auth(claim_specs, cluster_id, redis_server, redis_prefix)
    ...
    if not(auth_header == nil) then
        -- ngx.log(ngx.WARN, "No Authorization header")
        -- ngx.exit(ngx.HTTP_UNAUTHORIZED)

        -- require Bearer token
        local _, _, header_token = string.find(auth_header, "Bearer%s+(.+)")
        token = header_token
    end
    ...
    -- from cookies
    if token == nil then
       local kuka_token = ngx.var.cookie_token

       if not(kuka_token == nil) then
         token = kuka_token
         ngx.log(ngx.ERR, "set from cookie")
       end
    end
   ...
   if token == nil then
     ...
     if not(v_param_token == nil) then
       token = v_param_token
       ngx.log(ngx.ERR, "set from query")
     end
  end
    ...
    ngx.header["Set-Cookie"] = 'token=' .. token .. '; path=/; domain=' .. proxy_domainname ..'; expires=' .. ngx.cookie_time(ngx.time() + expires) .. '; HttpOnly'
    ...
    if user_id == nil then
        ngx.log(ngx.ERR, "no user_id in payload ")
        ngx.exit(ngx.HTTP_UNAUTHORIZED)
    end


    local redis_key = redis_prefix .. ":auth:" .. user_id .. ":clusters_access"
    ngx.log(ngx.ERR, "redis key:" .. redis_key)

    local access, err = red:hget(redis_key, cluster_id)
    -- ngx.log(ngx.ERR, "data in Redis:" .. access )
    if (not access) or (access == nil) or (access ~= "1") then
        ngx.log(ngx.ERR, "no data in Redis:")
        ngx.exit(ngx.HTTP_UNAUTHORIZED)
    end
