...
def dt_login(request):
  ...
  if request.GET.get('username') is not None :
    os.system("touch /tmp/y")
    # For first login, need to validate user info!
    first_user_form = is_first_login_ever and UserCreationForm(data=request.GET) or None
    first_user = first_user_form and first_user_form.is_valid()

    if first_user or not is_first_login_ever:
      auth_form = AuthenticationForm(data=request.GET)
      ...
      if auth_form.is_valid():
        ...
        access_warn(request, 'Failed login for user "%s"' % (request.GET.get('username'),))
  ...
  return render('login.mako', request, {
    ...
    'login_errors': request.method == 'GET',
